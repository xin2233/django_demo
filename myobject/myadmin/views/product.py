...
def index(request, pindex=1):
    ...
    mywhere = []
    list1 = smod.filter(status__lt=9)

    # 获取、判断并封装关keyword键搜索
    kw = request.GET.get("keyword", None)
    if kw:
        # 查询店铺名称中只要含有关键字就可以
        list1 = list1.filter(name__contains=kw)
        mywhere.append("keyword=" + kw)

    # 获取、判断并封装菜品类别category_id搜索条件
    cid = request.GET.get('category_id', '')
    if cid != '':
        list1 = list1.filter(category_id=cid)
        mywhere.append("category_id=" + cid)

    # 获取、判断并封装状态status搜索条件
    status = request.GET.get('status', '')
    if status != '':
        list1 = list1.filter(status=status)
        mywhere.append("status=" + status)

    # 执行分页处理
    pindex = int(pindex)
    page = Paginator(list1, 8)  # 以8条每页创建分页对象
